...
from utils.losses import TripletLoss, CrossEntropyLabelSmooth,InfoNce,ArcFace
from utils.utils import AverageMeter, Logger, save_checkpoint, print_time,DeepSupervision
from utils.eval_metrics import evaluate,evaluate_reranking
...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='vtn', help="c2resnet50, nonlocalresnet50")
parser.add_argument('--pretrain', action='store_true', help="load params form pretrain model on kinetics")
parser.add_argument('--pretrain_model_path', type=str, default='resnest50-528c19ca.pth', metavar='PATH')
# Miscs
...
def specific_params(args):
    if args.arch in ['make_model','motion_model','motion_model_v1']:
...
def main():
    ...
    criterions = {
        ...
        'xent_label': CrossEntropyLabelSmooth(num_classes=dataset.num_train_pids),
        'infonce':InfoNce(num_instance=args.num_instances),
        'arcface':ArcFace(s=32,margin=args.margin)}
    ...
    infer_epochs = [200]
    if best_epoch !=200: infer_epochs.append(best_epoch)
...
def train(epoch, model, criterions, optimizer, trainloader, use_gpu):
    ...
    batch_xent_loss_frame=AverageMeter()
    batch_htri_loss_frame = AverageMeter()
    ...
    for batch_idx, (vids, pids, camid) in enumerate(trainloader):
        ...
        i,y,f,cls_frame,trip_frame= model(vids)
        
        xent_loss_frame=DeepSupervision(criterions['xent_label'] , cls_frame,  pids , mode='CE-frame')
        htri_loss_frame=DeepSupervision(criterions['htri'] , trip_frame,  pids , mode='Trip-frame')
      
 
        xent_loss = criterions['xent_label'](y, pids)
        htri_loss = criterions['htri'](f, pids)
        info_loss = criterions['infonce'](i)

        loss=xent_loss+htri_loss+xent_loss_frame+htri_loss_frame+0.01*info_loss
        ...
        batch_xent_loss_frame.update(xent_loss_frame.item(), pids.size(0))
        batch_htri_loss_frame.update(htri_loss_frame.item(), pids.size(0))
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    # #save qf,gf feature 
    # torch.save(qf, '/home/cv-mot/zhw/Res_Cross_atten_VTN/feature_result/query_feature.pt')
    # torch.save(gf, '/home/cv-mot/zhw/Res_Cross_atten_VTN/feature_result/gallery_feature.pt')
    # scipy.io.savemat('/home/cv-mot/zhw/Res_Cross_atten_VTN/feature_result/save_result.mat', {'query_cam': q_camids,
    #                                'query_label': q_pids,
    ...
    qf = F.normalize(qf, dim=1, p=2)
    gf = F.normalize(gf, dim=1, p=2)
    # #cmc, mAP = evaluate_reranking(distmat,qf, q_pids, q_camids, gf, g_pids, g_camids, ranks)
    cmc, mAP = evaluate_reranking(distmat,qf, q_pids, q_camids, gf, g_pids, g_camids, ranks)
    _eval_format_logger(cmc, mAP, ranks, '')
    return cmc[0]
